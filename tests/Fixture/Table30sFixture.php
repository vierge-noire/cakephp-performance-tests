<?php
declare(strict_types=1);

namespace App\Test\Fixture;

use Cake\TestSuite\Fixture\TestFixture;

/**
 * Table30sFixture
 */
class Table30sFixture extends TestFixture
{
    /**
     * Fields
     *
     * @var array
     */
    // phpcs:disable
    public $fields = [
        'id' => ['type' => 'integer', 'length' => null, 'unsigned' => false, 'null' => false, 'default' => null, 'autoIncrement' => true, 'precision' => null, 'comment' => null],
        'name' => ['type' => 'string', 'length' => 128, 'null' => false, 'default' => 'Default name for table 30', 'precision' => null, 'comment' => null, 'collate' => null],
        'created' => ['type' => 'timestamp', 'length' => null, 'null' => false, 'default' => 'CURRENT_TIMESTAMP', 'precision' => null, 'comment' => null],
        'modified' => ['type' => 'timestamp', 'length' => null, 'null' => true, 'default' => null, 'precision' => null, 'comment' => null],
        '_constraints' => [
            'primary' => ['type' => 'primary', 'columns' => ['id'], 'length' => []],
        ],
    ];
    // phpcs:enable
    /**
     * Init method
     *
     * @return void
     */
    public function init(): void
    {
        $this->records = [
            [
                'id' => 1,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 2,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 3,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 4,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 5,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 6,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 7,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 8,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 9,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 10,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 11,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 12,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 13,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 14,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 15,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 16,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 17,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 18,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 19,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 20,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 21,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 22,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 23,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 24,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 25,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 26,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 27,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 28,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 29,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
            [
                'id' => 30,
                'name' => 'Lorem ipsum dolor sit amet',
                'created' => 1599835376,
                'modified' => 1599835376,
            ],
        ];
        parent::init();
    }
}
